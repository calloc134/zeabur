...
![dockerのデプロイ](/deploy/dockerfile/docker-deploy.png)
...
もし `Dockerfile` が 1 つの段階で書かれているか、あるいは環境変数が Docker の最終ステージでのみ使用される必要がある場合は、手動で `ENV` を書く必要はありません。Zeabur は自動的に環境変数を追加します。

### 引数

`Dockerfile` が複数の段階で書かれており（[マルチステージ](https://docs.docker.com/build/building/multi-stage/)）、ビルド前に環境変数を設定する必要がある場合は、`ARG` を使用して設定することができます。
...
"Redeploy" をクリックしてプロジェクトを再デプロイすると、希望のサービスがデプロイされます。また、[ウォッチパス](./watch-paths) を設定して、該当するファイルが変更された場合にのみデプロイをトリガーすることもできます。
